...
export default function Home(){
  ...
  function touchOrb(){setOrbHint(false);setShape(s=>(s+1)%5);setPalette(p=>(p+1)%palettes.length);setNote(n=>(n+1)%notes.length)}
  function bounceReaction(){setDvdHint(false);setDvdBurst(n=>n+1);setDvdPalette(p=>(p+1)%palettes.length);setDvdNote(n=>(n+1)%notes.length)}
  function reverseConstellation(){setConstellationHint(false);setConstellationReverse(v=>!v);setConstellationPalette(p=>(p+1)%palettes.length);setConstellationNote(n=>(n+1)%notes.length)}
  return <main>
    ...
    <section className="studio" id="studio">
      <div className={`orb-stage ${palettes[palette]}`}>
        ...
        {orbHint&&<span className="tap-cue tap-cue-one" aria-hidden="true">tap!</span>}
