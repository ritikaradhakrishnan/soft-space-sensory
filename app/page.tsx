...
export default function Home(){
  const [shape,setShape]=useState(0),[palette,setPalette]=useState(0),[note,setNote]=useState(0),[noise,setNoise]=useState<NoiseName>("pink"),[playing,setPlaying]=useState(false),[volume,setVolume]=useState(28),[rippleCount,setRippleCount]=useState(1),[ripplePalette,setRipplePalette]=useState(0),[constellationReverse,setConstellationReverse]=useState(false),[constellationPalette,setConstellationPalette]=useState(0);
  ...
  function splitRipple(){setRippleCount(n=>n>=20?1:n+1);setRipplePalette(p=>(p+1)%palettes.length)}
  function reverseConstellation(){setConstellationReverse(v=>!v);setConstellationPalette(p=>(p+1)%palettes.length)}
  return <main>
    ...
    <section className={`ripple-world ${palettes[ripplePalette]}`} aria-label="Liquid ripple orb sensory animation"><div className="ripple-ring rr-one"/><div className="ripple-ring rr-two"/><div className="ripple-ring rr-three"/><button className="ripple-cluster" onClick={splitRipple} aria-label={`Split the liquid orb. ${rippleCount} of 20 orbs`}>{Array.from({length:rippleCount},(_,i)=>{const angle=i*2.4,radius=i*10,size=rippleCount===1?440:Math.max(58,280-rippleCount*11);return <span className="liquid-orb" key={i} style={{"--r":i,width:`${size}px`,left:`calc(50% + ${Math.cos(angle)*radius}px)`,top:`calc(50% + ${Math.sin(angle)*radius}px)`} as React.CSSProperties}><i/><b/></span>})}</button><span className="ripple-dot rd-one"/><span className="ripple-dot rd-two"/></section>
    ...
    <section className={`constellation-world ${constellationReverse?"reverse":""} ${palettes[constellationPalette]}`} aria-label="Floating pink orb constellation sensory animation"><button className="constellation-field" onClick={reverseConstellation} aria-label={`Reverse the constellation. Currently moving ${constellationReverse?"counterclockwise":"clockwise"}`}><div className="constellation-core"/>{Array.from({length:14},(_,i)=><span className="constellation-orb" key={i} style={{"--c":i} as React.CSSProperties}><i/></span>)}<div className="constellation-path cp-one"/><div className="constellation-path cp-two"/></button></section>
