...
export default function Home(){
  const [shape,setShape]=useState(0),[palette,setPalette]=useState(0),[note,setNote]=useState(0),[noise,setNoise]=useState<NoiseName>("pink"),[playing,setPlaying]=useState(false),[volume,setVolume]=useState(28),[dvdBurst,setDvdBurst]=useState(0),[dvdPalette,setDvdPalette]=useState(0),[constellationReverse,setConstellationReverse]=useState(false),[constellationPalette,setConstellationPalette]=useState(0);
  ...
  function bounceReaction(){setDvdBurst(n=>n+1);setDvdPalette(p=>(p+1)%palettes.length)}
  ...
  return <main>
    ...
    <section className={`dvd-world ${palettes[dvdPalette]} hit-${dvdBurst%2}`} aria-label="Bouncing pink orb sensory animation"><button className="dvd-stage" onClick={bounceReaction} aria-label="React to the bouncing orb and change its color"><span className="dvd-x"><span className="dvd-y"><span className="dvd-orb"><i/><b/></span></span></span><span className="impact" key={`impact-${dvdBurst}`}>{Array.from({length:12},(_,i)=><i key={i} style={{"--p":i} as React.CSSProperties}/>)}</span><span className="screen-ring sr-one"/><span className="screen-ring sr-two"/></button></section>
